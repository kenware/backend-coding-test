...
describe('GET /Test sqlinjections', () => {
  it('should return bad response with sql injection', (done) => {
    request(app)
      .get(`/rides/delete from Rides where rideID=${rideID}`)
      .expect('Content-Type', 'application/json; charset=utf-8')
      .expect(function(res, err) {
        const message = 'Could not find any ride';
        assert(res.body.error_code, 'VALIDATION_ERROR');
